var url =
  'https://www.googleapis.com/webfonts/v1/webfonts?key=AIzaSyDzNP2S2VIo_jiErF8L--b6V8bkaKoQNQg';
  ...
  $.getJSON(url, function (json) {
    $.each(json.items, function (i, type) {
      if (type.family === family) {
        ...
        details.append(
          "<div class='detail-title'>Variants</div><ul class='list-variant'></ul>"
        );
        $(type.variants).each(function (index, item) {
          $('.list-variant').append($(document.createElement('li')).text(item));
        });
        details.append(
          "<div class='detail-title'>Subsets</div><ul class='list-subsets'></ul>"
        );
        $(type.subsets).each(function (index, item) {
          $('.list-subsets').append($(document.createElement('li')).text(item));
        });
        details.append(
          "<div class='detail-title horizontal'>Category</div><div class='detail-data'>" +
            type.category +
            '</div>'
        );
        details.append(
          "<div class='detail-title horizontal'>Version</div><div class='detail-data'>" +
            type.version +
            '</div>'
        );
        details.append(
          "<div class='detail-title horizontal'>Last Modified</div><div class='detail-data'>" +
            type.lastModified +
            '</div>'
        );
        details.append(
          "<div class='detail-title'>HTML</div><div class='detail-data'><pre><code class='language-markup'>&lt;link href='" +
            familyCSS +
            "' rel='stylesheet' type='text/css'&gt;</code></pre></div>"
        );
        details.append(
          "<div class='detail-title'>CSS</div><div class='detail-data'><pre><code class='language-css'>font-family: '" +
            family +
            "', sans-serif;</code></pre></div>"
        );
        details.append(
          "<a href='https://fonts.google.com/specimen/" +
            familyPlus +
            "' class='btn'>View on Google Fonts &rarr;</a>"
        );
        ...
        if (details.text().match('700')) {
          $('head').append(
            '<style>.main strong,h1,h2,h3 { font-weight: 700; }'
          );
        } else if (details.text().match('800')) {
          $('head').append(
            '<style>.main strong,h1,h2,h3 { font-weight: 800; }'
          );
        } else if (details.text().match('900')) {
          $('head').append(
            '<style>.main strong,h1,h2,h3 { font-weight: 900; }'
          );
        }
        ...
        $.each(visited, function (i, val) {
          visit +=
            "<li class='link-history' data-family='" +
            val +
            "'>" +
            val +
            '</li>';
